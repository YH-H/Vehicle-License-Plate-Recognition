...
def accurate_place(card_img_hsv, limit1, limit2, color):#颜色定位大致范围
...
def img_Transform(car_contours, oldimg, pic_width, pic_hight):
    ...
    for car_rect in car_contours:
        if -1 < car_rect[2] < 1:#创造角度，使得左、高、右、低拿到正确的值
            angle = 1
            # 对于角度为-1 1之间时，默认为1
        else:
            angle = car_rect[2]
        car_rect = (car_rect[0], (car_rect[1][0] + 5, car_rect[1][1] + 5), angle)#扩大范围，避免车牌边缘被排除
